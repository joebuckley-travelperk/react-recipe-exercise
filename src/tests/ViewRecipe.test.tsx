...
const setup = () => {
  const utils = render((<MemoryRouter><ViewRecipe/></MemoryRouter>))
...
it('renders correctly', async () => {
  ...
  await act(async () => {
    tree = renderer
    .create(<MemoryRouter><ViewRecipe/></MemoryRouter>)
    .toJSON();
...
test('render mock data', async () => {
  ...
  const recipeElement =  await waitFor(() => screen.getByText(mockData.data[0].name))
