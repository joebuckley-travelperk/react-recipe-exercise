...
test("retieve one element", async () => {
  ...
  const successElement = await waitFor(() =>
    screen.getByText(new RegExp(mockData.data.name, "i"))
  );
...
test("submit update field", async () => {
  ...
  await act(async () => {
    ...
    fireEvent.change(ingredientInput, {
      target: { value: mockDataUpdate.data.ingredients[0].name },
    });

    const { nameInput, descriptionInput } = getInputs(utils);
    fireEvent.change(nameInput, {
      target: { value: mockDataUpdate.data.name },
    });
    fireEvent.change(descriptionInput, {
      target: { value: mockDataUpdate.data.description },
    });
  ...
  const oldIngredient = await screen.queryByText(
    new RegExp(mockData.data.ingredients[0].name, "i")
  );
  const newIngredient = await screen.getByText(
    new RegExp(mockDataUpdate.data.ingredients[0].name, "i")
  );
  const newName = await screen.queryByText(
    new RegExp(mockDataUpdate.data.ingredients[0].name, "i")
  );
  const newDescription = await screen.queryByText(
    new RegExp(mockDataUpdate.data.ingredients[0].name, "i")
  );
...
test("delete element successful", async () => {
  RecipeService.getOne = jest.fn(() => Promise.resolve(mockData));
  RecipeService.delete = jest.fn(() =>
    Promise.resolve({ data: {} as IRecipe, status: 204 })
  );
...
test("delete element failure", async () => {
  RecipeService.getOne = jest.fn(() => Promise.resolve(mockData));
  RecipeService.delete = jest.fn(() =>
    Promise.resolve({ data: {} as IRecipe, status: 400 })
  );
