...
function RecipeForm({
  ...
  const checkInvaildInputs = () => {
    ...
    return recipe.ingredients.reduce(
      (isEmpty: InputName, ingredient: IIngredient) => {
  ...
  useEffect(() => {
    ...
    else if (success !== undefined && handleFormUpdate)
      handleFormUpdate(recipe);
  }, [success]);
  ...
  useEffect(() => {
    if (isEdit && formerRecipe.name) {
      ...
      const ingredients = formerRecipe.ingredients.map((recipe, index) => ({
        name: recipe.name,
        index: String(index),
      }));
      setIngredientInputs(ingredients);
  ...
  const handleAddIngredientInput = () => {
    setIngredientInputs((ingredient) => [
      ...ingredient,
      { name: "", index: String(ingredient.length + 1) },
    ]);
  };

  const handleDeleteIngredient = (index: string) => {
    const ingredients = ingredientInputs.filter(
  ...
  return (
    <div>
          ...
          return (
            <div key={index}>
              ...
              <Button
                type="button"
                onClick={() =>
                  handleDeleteIngredient(ingredient.index as string)
                }
                color="danger"
              >
                Delete Ingredient
      ...
      <ButtonField>
        <Button color="primary" onClick={handleAddIngredientInput}>
          Add Ingredient
        </Button>
      </ButtonField>
